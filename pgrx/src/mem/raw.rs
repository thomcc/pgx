use crate::pg_sys::{self, CurrentMemoryContext, MemoryContext};
use core::{
    ffi::{c_void, CStr},
    marker::PhantomData,
    ptr::NonNull,
...
unsafe fn memory_context_is_valid(p: *mut MemoryContextData) -> bool {
    !p.is_null()
        && (crate::is_a(p.cast(), pg_sys::NodeTag_T_AllocSetContext)
            || crate::is_a(p.cast(), pg_sys::NodeTag_T_SlabContext)
            || crate::is_a(p.cast(), pg_sys::NodeTag_T_GenerationContext))
}
